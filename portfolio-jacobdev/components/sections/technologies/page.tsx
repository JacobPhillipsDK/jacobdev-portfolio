...
export default function TechnologyTabs() {
  ...
  return (
      <div className="w-full py-16 lg:py-16" id="technologies">
            ...
            <TabsList className="grid w-full grid-cols-2 text-sm sm:grid-cols-3 sm:text-base lg:grid-cols-5 lg:text-lg">
              {tabs.map((tab) => (
                  <TabsTrigger
                      key={tab.id}
                      value={tab.id}
                      className="rounded-lg dark:data-[state=active]:bg-white dark:data-[state=active]:text-primary-foreground"
                  >
                    {tab.label}
                  </TabsTrigger>
              ))}
...
function TechnologyGrid({ tabId }: TechnologyGridProps) {
  ...
  return (
      <Card className="w-full max-w-none bg-background">
